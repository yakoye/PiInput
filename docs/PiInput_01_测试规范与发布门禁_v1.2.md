...
> Test Specification & Release Gate · v1.4
> 更新日期：2026-08-26
...
| v1.3 | 增加候选动作、自定义快捷调用和随包 HTML 运行时资产门禁 | 2026-08-25 |
| v1.4 | 增加动态快捷表、内置行、英文候选动作、Windows 工具模板及第三方模板许可门禁 | 2026-08-26 |
...
| 领域 | 当前状态 | 已有证据 | 仍缺少 |
|---|---|---|---|
| 核心引擎/Host/TSF | 较完整 | 当前 Release 构建和 66/66 CTest 已通过；新增候选动作及既有 Controlled TSF Host、UAC manifest 自检均纳入全量回归 | 真实宿主矩阵和下一冻结候选长时验证；不得用组件测试替代 |
...
- Release 构建成功。
- 当前 CMake 注册测试全部执行，不能只报告“已注册若干项”。
...
- G0/G1：当前标准 Release 构建和 66/66 CTest 已通过。
...
- 内置和自定义启动入口必须经过独立动作类型，不得把候选标签提交到正文或写入用户学习。
- 动作只能在最终 TSF edit 成功后执行；失败必须保留或恢复原组合，不得先抢焦点再清理输入状态。
- 每个内置别名、候选序号、动作目标和误触发反例必须进入 L0/L1 回归；撤销的别名不得残留隐藏兼容分支。
- 快捷表必须覆盖动态 0～64 行、五字段往返、添加/编辑/删除、旧三槽迁移、删除后 stale key 不复活和恢复默认；内置入口必须在表中可见。
- 英文候选开启时必须保留原始英文为候选 1 并允许动作选择；关闭时不得延迟或截获英文字母，Shift 切中文是唯一回退方式。
- Windows 工具模板只作为可选库，导入后转换为普通行；带参数目标必须分离程序和参数。引用的模板许可必须进入发布包，Everything 等第三方程序不得无授权打包。
- 随包 HTML 及其 CSS/JS 属于运行时资产，必须同时进入安装目录、发布白名单和包闭环必需文件检查；缺少任一文件时 G7 失败。
- 自动协议测试不能替代计算器、画图、HTML 和自定义目标在同一已安装候选上的真实启动验收。
