...
用户在 v0.6.4（已确认安装的 Host 与 Shim 哈希均为 v0.6.4）上录屏。以 30 fps 逐帧对组合区与候选栏分别做哈希比对：

- 候选栏与组合区文字的更新相差 1～2 帧，说明候选栏显示时机已经不是瓶颈；
- 「我们，」提交后，第一个字母 `n` **没有任何一帧单独出现**，`ni` 两个字母在同一帧一起显示；
- 同一段里词中间的 `niv`（第 100 帧）→ `nivi`（第 101 帧）是逐帧单独出现的。

需要说明：录像无法给出按键的物理时刻，因此无法直接测得按键到显示的绝对延迟；上面用的是「首字母是否单独成帧」这一相对判据。

### 根因

`FinalEditKeyQueue::should_queue()` 只要 `final_edit_pending_` 为真就拦下所有按键。该标志由提交回复置位，直到那次 TSF 编辑完成才清除。`TextService::handle_reply()` 的提交分支在 `request_edit(...)` 返回 `pending` 时并不调用 `complete_final_edit()`，而是交给 `complete_deferred_edit()`——也就是等应用下一轮消息循环授予异步编辑会话之后。Notepad++ 经常拒绝同步编辑会话（v0.4.6 的异步回退正是为此存在），于是提交之后打下的每个按键都被排队，再逐个放行，每个一次完整往返。

普通汉字字母无需等待：其回复是 `update`，TSF 编辑已由 `DeferredUpdateQueue` 排在挂起的提交之后，且多次更新会合并为最新的一次。

### 修复

新增 `key_may_begin_final_edit(HostKeyKind, bool english_mode)`：

- 返回假：`text`（中文模式）、左右移动、行首行尾、候选左右移动、行展开与上一行；
- 返回真：标点、空格、回车、Esc、数字选字、候选选择、中英切换、退格、删除，以及英文直输模式下的 `text`。

`should_queue(event_may_begin_final_edit)` 变为：队列非空或有替补按键在途时仍然排队以保持顺序；否则只有「可能再次开始最终编辑」的按键才需要等待挂起的提交。

### 回归

`composition_mirror_tests` 扩充：挂起提交时普通字母不排队、可提交的按键仍排队、队列非空后一律排队保持顺序，以及 `key_may_begin_final_edit` 的分类断言。
