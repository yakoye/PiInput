...
- SearchHost 后续验收发现候选窗仍不显示，而最终文本正常。第一层根因是 key reply 与 caret 管道的并发顺序，已按 `client + session + generation` 精确消费提前到达的完整结果。继续实机复测后确认还有第二层根因：Host 原来只创建无 owner 的外部 `WS_POPUP`，Windows 搜索等系统面板不保证它位于宿主表面上方。当前协议升级到 v4；Shim 在 edit session 内通过 `ITfContextView::GetWnd` 取得文本视图，失败时回退 `GetFocus`，统一到 `GA_ROOT` 后把 HWND 传给 Host；Host 验证句柄并以该根窗口作为 `CreateWindowExW` 的 owner，owner 变化或失效时重建候选窗。L2 已覆盖 wire 兼容、子控件归一、真实 `GW_OWNER` 和宿主销毁重建；Controlled TSF smoke 也必须观察到可见候选窗的 owner 等于测试宿主。新构建的 SearchHost 候选 UI 仍待安装后实机验收，不能提前标为 PASS。
