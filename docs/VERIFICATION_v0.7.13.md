...
- 已定位并修复 Windows 搜索无法中文输入：现场模块枚举显示 SearchHost 已加载搜狗 TSF，但未加载 PiInput；机器 profile、`IMMERSIVESUPPORT` 类别和 DLL 的 AppContainer ACL 均存在，缺口是安装器只写 HKCU COM，而打包系统宿主需要 64 位 HKLM COM 才能创建 TIP。提权事务现先把 Shim 部署到 Program Files 的固定受保护路径，再原子注册 profile、capability category 和 HKLM `InprocServer32`；HKCU 指向同一机器 Shim，避免让机器 COM 加载用户可写 DLL。失败恢复此前机器 COM，卸载对称清理注册和机器运行目录。诊断 JSON 与包闭环同时核对 HKCU/HKLM 路径、存在性、哈希和卸载残留。`0.7.13+ed980905fcb4` 已在本机 SearchHost 实际加载并可输入拼音、以空格提交汉字。
- SearchHost 后续验收发现候选窗仍不显示，而最终文本正常。此前修复了 key reply/caret 竞态和外部 popup owner，但实机仍失败，说明跨进程 z-order 不是最终根因。深层根因是安装注册已声明 `UIELEMENTENABLED/IMMERSIVESUPPORT`，Shim 却只实现 `ITfTextInputProcessor` 和外部 Host popup，没有向 UI-less/集成式宿主发布候选 UIElement。当前已补齐 `ITfTextInputProcessorEx::ActivateEx`、`ITfCandidateListUIElementBehavior` 与 `ITfIntegratableCandidateListUIElement`；协议升级到 v5，`BeginUIElement` 返回宿主接管时 Shim 发送 `show_candidate_window=false`，Host 隐藏外部窗，由 Windows 搜索渲染系统候选行。普通桌面宿主仍使用 v4 已有的 owned popup 路径。L0/L2 已覆盖候选字符串、分页、选择/取消、搜索框 integration style、v4 兼容和显示通道位；新构建仍须安装后在 SearchHost 看到首字母系统候选行，不能提前标为 PASS。
- 首版 UIElement 实机复测又暴露一个完整生命周期错误：Shim 在 `BeginUIElement` 之前填入首批候选，却没有在 Begin 成功后发送首个 `UpdateUIElement`；Windows 的 UIElement sink 已返回“由宿主显示”，所以 Host 正确隐藏了外部窗，但系统从未收到首批候选已就绪的通知，Notepad++ 与 SearchHost 都会出现“有预编辑、无候选”。当前顺序已改为 `BeginUIElement -> 写入首批 snapshot -> UpdateUIElement`，与 Microsoft SampleIME 的注册后更新生命周期一致，并增加源码门禁固定该顺序。该修复尚未由用户在新安装包上完成 SearchHost 可见性验收，状态仍不得写 PASS。
- 数字后句号已按用户确认恢复确定语义：第一下立即输出 ASCII `.`，第二下输出中文 `。`；`1.文本` 不得再被异步回写成 `1。文本`，`1..` 应得到 `1.。`。L0 与 Controlled TSF 物理键 Oracle 已更新，真实宿主待新构建复验。
