...
安装器首次注册永久稳定的 TSF 入口，之后普通升级只切换独立的 `PiInputHost.exe`，**不再要求关闭正在使用的程序**。它同时完成旧用户数据迁移、失败回滚和原生卸载注册；交互安装完成后会打开用户配置目录和 `PiInput-Settings.exe`。详见 [安装与使用指南](docs/安装与使用指南.md) 与 [稳定入口与无重启升级说明](docs/稳定入口与无重启升级说明.md)。
