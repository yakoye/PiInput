...
Copy-Item -LiteralPath (Join-Path $Root "query-dictionary.cmd") -Destination $PackageRoot
New-Item -ItemType Directory -Force (Join-Path $PackageRoot "scripts/windows") | Out-Null
Copy-Item -LiteralPath (Join-Path $Root "scripts/query-dictionary.ps1") -Destination (Join-Path $PackageRoot "scripts")
Copy-Item -LiteralPath (Join-Path $Root "scripts/windows/resolve-installed-dev.ps1") -Destination (Join-Path $PackageRoot "scripts/windows")
