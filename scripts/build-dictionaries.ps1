...
if (-not (Test-Path $Builder) -or -not (Test-Path $Compiler) -or -not (Test-Path $Converter)) {
    Write-Host "Building LiteIME dictionary tools..." -ForegroundColor Cyan
...
try {
    ...
    $arguments.Add("--source"); $arguments.Add("phrase-pinyin-data"); $arguments.Add((Join-Path $Sources "phrase-pinyin-data/large_pinyin.txt")); $arguments.Add("5000")
